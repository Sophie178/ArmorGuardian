...
{
    public partial class CargoEsc : Form
    {
        ...
        string query = "select * from CargoEscorting";

        string table = "CargoEscorting";
        ...
        private void TariffGrid_UserDeletingRow(object sender, DataGridViewRowCancelEventArgs e)
        {
            ...
            {
                if (MessageBox.Show("Delete this record?", "Cargo escorting services table", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    string id = TariffGrid.CurrentRow.Cells["TariffID"].Value.ToString();
                    connection.Open();
                    string query2 = $"delete from CargoEscorting where TariffID = {id}";
        ...
        private CargoClass ValidateCEService()
        {
            ...
            if (Convert.ToInt32(TariffGrid.CurrentRow.Cells[_mHours].Value.ToString()) <= 24)
            {
                ...
                if (TariffGrid.CurrentRow.Cells[_transport].Value.ToString() == "")
                {
                    trans = 0;
                }
                else
                ...
                CargoClass csService = new CargoClass(tariff, name, maxHours, trans, mPrice, arm, desc, guards);
                return csService;
            }
            else
            {
        ...
        private void Updating(string table, CargoClass ceService)
        {
            if (MessageBox.Show("Edit this record?", $"{table} table", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                string query2 = $"update {table} set  " +
                     $@" {_nId} = '{ceService.Name}', " +
                    $" {_mHours} = {ceService.MaxHours}, " +
                    $" {_transport} = {ceService.Transport}, {_mPrice} = {ceService.MinPrice}, {_armed} = {ceService.Armed}, {_desc} = '{ceService.Description}' ," +
                    $"  {_guards} = {ceService.Guards} " +
                    $"where  {_tId} = {ceService.TariffID}";
                SqlCommand sqlCommand = new SqlCommand(query2, connection);
                try
                {
                    sqlCommand.ExecuteNonQuery();
        ...
        private void Adding(string table, CargoClass ceService)
        {
            if (MessageBox.Show("Add this record?", $"{table} table", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                string query2 = $"insert into {table} values ({ceService.TariffID}, " +
                     $@" '{ceService.Name}', " +
                    $" {ceService.MaxHours},  " +
                    $" {ceService.Transport}, {ceService.MinPrice}, {ceService.Armed}, '{ceService.Description}', {ceService.Guards})";
                SqlCommand sqlCommand = new SqlCommand(query2, connection);
                try
                {
                    sqlCommand.ExecuteNonQuery();
        ...
        private void UpdateButton_Click(object sender, EventArgs e)
        {
            try
            {
                if (ValidateCEService() != null && ValidateCEService().Description != "")
                    Updating(table, ValidateCEService());
        ...
        private void AddButton_Click(object sender, EventArgs e)
        {
            ...
            {
                if (ValidateCEService() != null && ValidateCEService().Description != "")
                    Adding(table, ValidateCEService());
                else { MessageBox.Show("Empty cells are not allowed"); }
            }
            catch
            {
